...
def parse_args():
    ...
    parser.add_argument('-C','--config', default=globals.CONFIG, required=False, help='Name of the input config file')
...
def parse_spw(filename):
    ...
    if ',' in spw:
        ...
        for i,SPW in enumerate(SPWs):
            low[i],high[i],unit[i],func = kwarg_tools.get_spw_bounds(SPW)
        ...
        low,high,unit,func = kwarg_tools.get_spw_bounds(spw)
        dirs = []

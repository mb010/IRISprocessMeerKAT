...
#See globals.py for license details.
...
def read_ms(MS):
    ...
    if default.size == 0:
        logger.logger.error('You must have a field with intent "CALIBRATE_FLUX". I only found {0} in dataset "{1}".'.format(msmd.intents(),MS))
        return fieldIDs
...
def get_field(MS,intent,fieldname,extra_fields,default=0,multiple=False):
    ...
    if fields.size == 0:
        logger.logger.warn('Intent "{0}" not found in dataset "{1}". Setting to "{2}"'.format(intent,MS,default))
        fieldIDs = "'{0}'".format(default)
    elif fields.size == 1:
        fieldIDs = "'{0}'".format(fields[0])
    else:
        logger.logger.info('Multiple fields found with intent "{0}" in dataset "{1}" - {2}.'.format(intent,MS,fields))
        
        if multiple:
            logger.logger.info('Will use all of them for "{0}".'.format(fieldname))
            ...
            logger.logger.warn('Only using field "{0}" for "{1}", which has the most scans ({2}).'.format(maxfield,fieldname,maxscan))
            ...
            if len(extras) > 0:
                logger.logger.warn('Putting extra fields with intent "{0}" in "extrafields" - {1}'.format(intent,extras))
                extra_fields += extras
...
def check_refant(MS,refant,config,warn=True):
    ...
    if refant not in ants:
        ...
        logger.logger.info("Using reference antenna '{0}'.".format(refant))
        if refant == 'm059':
            logger.logger.info("This is usually a well-behaved (stable) antenna. Edit '{0}' to change this, by updating 'refant' in [crosscal] section".format(config))
            logger.logger.debug("Alternatively, set 'calcrefant=True' in [crosscal] section of '{0}', and include 'calc_refant.py' in 'scripts' in [slurm] section.".format(config)) #(included by default)
...
def check_scans(MS,nodes,tasks,dopol):
    ...
    if abs(nodes * tasks - limit) > 0.1*limit:
        logger.logger.warn('The number of threads ({0} node(s) x {1} task(s) = {2}) is not ideal compared to the number of scans ({3}) for "{4}".'.format(nodes,tasks,nodes*tasks,nscans,MS))
        ...
        while nodes * tasks < limit:
            if nodes < globals.TOTAL_NODES_LIMIT:
                nodes += 1
            elif tasks < globals.NTASKS_PER_NODE_LIMIT:
                tasks += 1
            else:
                break
    
        logger.logger.warn('Config file has been updated to use {0} node(s) and {1} task(s) per node.'.format(nodes,tasks))
        if nodes*tasks != limit:
            logger.logger.info('For the best results, update your config file so that nodes x tasks per node = {0}.'.format(limit))

    if nodes > 4:
        logger.logger.warn("Large allocation of {0} nodes found. Please consider setting 'createmms=False' in config file, if using large number of SPWs.".format(nodes))
...
def check_spw(config):
    ...
    if nspw > 1:
        logger.logger.warn("Expected 1 SPW but found nspw={0}. Please manually edit 'spw' in '{1}'.".format(nspw,config))
    ...
    if update:
        logger.logger.warn('Default SPW outside SPW of input MS ({0}~{1}MHz). Forcing SPW={2}'.format(ms_low,ms_high,SPW))
...
def parang_coverage(vis, calfield):
    ...
    logger.logger.debug("Delta parang: {0}".format(delta_parang))
    tb.close()
...
def main():
    ...
    args = globals.parse_args()
    globals.setup_logger.logger(args.config,args.verbose)
    msmd.open(args.MS)
    
    dopol = args.dopol
    refant = config_parser.parse_config(args.config)[0]['crosscal']['refant']
    fields = read_ms(args.MS)
    logger.logger.info('[fields] section written to "{0}". Edit this section if you need to change field IDs (comma-seperated string for multiple IDs, not supported for calibrators).'.format(args.config))
    
    npol = msmd.ncorrforpol()[0]
    parang = parang_coverage(args.MS, int(fields['phasecalfield'][1:-1])) #remove '' from field
    
    if npol < 4:
        logger.logger.warn("Only {0} polarisations present in '{1}'. Any attempted polarisation calibration will fail, so setting dopol=False in [run] section of '{2}'.".format(npol,args.MS,args.config))
        dopol = False
    elif parang < 30:
        logger.logger.warn("Parallactic angle coverage is < 30 deg. Polarisation calibration will most likely fail, so setting dopol=False in [run] section of '{0}'.".format(args.config))
        dopol = False
